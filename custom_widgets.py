...
def label_plus_multiselect(label, options, ratio=(5,5), key=None, help=None, on_change=None):
    c1, c2 = st.columns(ratio)
    c1.write(label)
    multiselect = c2.multiselect(label, options, label_visibility="collapsed", key=key, help=help, on_change=on_change)
    return multiselect
